...
- **Observability** — three signals out of `src/observability/`.
  ...
  **Traces**: `traced_node` spans per agent behind `enable_tracing`
  (ADR
  [0013](decisions/0013-sprint-1-finish-retry-checkpoint-tracing-recall.md)).
  **Metrics**: **twenty-four** OTel instruments behind `enable_metrics`
  (ADR [0049](decisions/0049-otel-metrics.md), extended by ADRs 0051,
  [0066](decisions/0066-genai-semantic-conventions.md),
  [0081](decisions/0081-degradation-counter-and-the-quality-sli.md) and
  ...
  conventional `gen_ai.*` family, the two HTTP server RED
  instruments, and the contract event bridge's two (accepted
  trajectory events by lane, and the faults around them by stage and
  error code). That count is re-derived from `src/` by
