...
**Rows reviewed at `ed71098`** (`ed71098b2c7854f8608a9194c8f82201e941c87b`).
Every row was checked by opening the artifact at that commit, not by trusting a
filename, and the per-row **Reviewed** column records that commit rather than
today's — a row is only as fresh as the last time somebody read what it cites.
A row whose artifact path no longer resolves fails
`tests/test_assurance_docs.py`, so this page cannot rot into a list of dead
links without something going red.

**Citation sweep at `a3b112f`, 2026-09-17.** A narrower pass than a row review:
every cited path and every `file:line` was re-resolved against the tree and
three had drifted — the per-agent model fields and the `pyproject.toml`
advisory note had both moved, and one `README.md` range pointed at unrelated
prose. Those are corrected; the isolation-signature and safety-suite citations
still land where they did. The rows' *substance* still carries `ed71098`, because
nothing in this sweep re-read the artifacts to check that each one still says
what its row claims.
...
| AI RMF | **MEASURE 2.3** — system performance is measured against the deployment context. | Nothing is deployed and no funded campaign has run. The measured numbers this repository has are structural (tests, coverage, types) and adversarial (the safety suite); the three LLM-judged accuracy metrics have never produced a `summary.jsonl`. | [`system-card.md`](system-card.md) §5, `docs/eval.md` | `ed71098` | **Out-of-reach** — requires owner-approved model spend (W-OD-1) and a running deployment; both are standing constraints of this phase. |
| AI RMF | **MANAGE 3.2** — "Pre-trained models which are used for development are monitored as part of AI system regular monitoring and maintenance." | The model id is a typed setting per agent with an explicit fallback chain, and the eval provenance block records the product model and judge model on every row so two campaigns on different models cannot be silently compared. There is no *runtime* monitoring of the upstream model, because nothing is running. | `src/config.py:1327-1389`, `src/eval/provenance.py`, `src/eval/regression_diff.py` | `ed71098` | **Partial** — provenance and refusal-to-compare are in place; continuous monitoring of the hosted model is not, and cannot be until something is deployed. |
...
|---|---|---|---|---|
| `langgraph-checkpoint-sqlite` | 3.1.0 | PYSEC-2026-3636 | 3.1.1 | Already recorded in `pyproject.toml:35-39` — a namespace-scoping advisory in the *Store* implementations this project does not use, to be picked up at the next lock refresh. The audit confirms the note rather than discovering something new. |
