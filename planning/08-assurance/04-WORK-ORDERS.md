...
Fifteen work orders in three waves. Each is one branch, one PR, one
...
wave 3   A09  A10  A11  A12       A13 (CI, last)  A14 (evidence, last)
```
...
| A04 | Resilience policy: breaker, timeouts, dead-letter | 2 | 0068 | error handling |
...
## WO-A04 — Resilience policy: breaker, timeout budgets, dead-letter

**ADR:** 0068. **Branch:** `assurance/wo-a04-resilience`. **Depends:** A01.

### Objective

There is no circuit breaker anywhere, so during an upstream outage every job
pays its full retry envelope before failing. The Redis client has no connect
...
1. One breaker in `src/resilience.py`: closed/open/half-open, configured from
   settings, per-dependency instances, emitting an `upstream_*` `AppError`
   immediately while open. Pure and unit-testable — no global state that
   tests cannot reset.
2. Applied to: the model provider path, the arXiv HTTP session, the Semantic
   Scholar session. **A healthy system must behave exactly as before** — add a
   test that asserts the closed breaker is a pass-through.
3. Every timeout becomes a setting, including arXiv's hardcoded `30`.
...
- Breaker unit tests cover all three states and the half-open probe.
...
- Do not put the breaker inside `src/llm.py`'s client singleton; wrap the call
  site so per-job behaviour stays possible.
...
1. **Exact conventional names** from `02-STANDARDS.md` §1 — do not invent
   them, and do not rely on memory. Read that section, and if it disagrees
   with what you believe, follow the document and say so in your PR.
...
1. `stats.py`: paired bootstrap confidence intervals over tasks with
   hierarchical resampling when repeats exist; Wilson intervals for binary
   rates; all pure, seeded, and unit-tested against known inputs.
...
6. The report prints an interval and a decision, and states plainly when N is
   too small to separate a move from noise. That sentence is the deliverable —
...
1. A corpus under `tests/fixtures/safety/` with categories mapped to the OWASP
   LLM Top 10 (`02-STANDARDS.md` §3): direct injection, indirect injection via
   retrieved content, instruction override, exfiltration, tool/scope
   escalation, source laundering, poisoned metadata, cross-principal probing.
   Each case carries an id, category, the payload, the expected behaviour, and
   a provenance note. Author them; do not import a licensed external dataset.
2. **Behavioural assertions, not canary substrings**: assert what the agent
   *did* — which tools ran, which fields changed, what left the process, what
   ...
   fail.
3. **Attack success rate** as a reported metric with its denominator, plus
   **zero-tolerance classes** (cross-principal leakage, secret echo) that fail
...
1. SLI/SLO table per `03-ARCHITECTURE.md` §5.4, with error budgets and an
   ...
   and will be revised against measurement.
...
1. **Model card** for the deployed system: intended use, out-of-scope use,
   the models used and their routing, evaluation results **as measured**
   (never aspirational), known limitations, safety measures, and the explicit
   statement that judge–human calibration is unmeasured.
2. **Data provenance record**: the benchmark query set, learning scenarios,
   recorded mock sessions, safety corpus, and content packs — origin, author,
   license, date, and contamination notes, including the "well-covered by the
   built-in mock papers" annotation.
3. **Framework mapping** (`02-STANDARDS.md` §4): a table mapping NIST AI RMF
   functions, the OWASP LLM Top 10 categories, and the EU AI Act
   record-keeping expectations to the concrete artifact in this repository
   that satisfies each — and an honest **"not satisfied"** column. A mapping
   with no gaps in it is a mapping nobody checked.
...
5. **Gate A3 evidence pack** assembled from CI artifacts and runner-verified
   locally — never a number typed by hand.
