...
| **S2d — the report reader's mount reservation** | Branch `assurance/s2d-report-reader-reserve` (PR open) | The third member of the S2 family, and the first with no number to reserve. `ReportReader`'s loading state is seven skeleton lines — **216 px** at 412×915 — standing in for a briefing that renders at **844 px**, 1,161 px once its rail and metrics are in; below 767 px the thread is a document and not a frame (S2), so everything under the reading column moves when it grows. Reproduced exactly as F1 measured it, at the gated Pixel 7 profile on `thread-populated`: **0.00000 up to 6×, 0.04176 at 7×, 0.07944 at 8×–20×**, from `div.ew-thread__composer` going off screen (0.03768) and `article.ew-report` dropping 88 px when the section rail — `flex: 1 1 100%` below 1280 px, so it stacks *above* the document — arrived a commit late (0.04176). Two fixes, both S2c's mechanism rather than a reserved height: `ThreadTimeline` holds its loading frame until the Markdown pipeline has **settled** (`useReportPipeline`, which settles on a failed chunk too, so a 404 releases the frame rather than stranding the transcript), and `ReportReader` reads its headings in a **layout** effect. After: **0.00000 at 1×–7×, 0.00495 at 8×–20×** against the 0.02 ceiling; the other three audited states 0.00000 unchanged; desktop 1280×900 `thread-populated` **0.00112 → 0.00000**; three-checkpoint 0.00015/0.00004 unchanged at both widths. 3,558 unit tests across 165 files, 322 storybook tests, chromium 329 passed / 5 skipped, Pixel 7 21 passed, visual 55/55 twice with **no golden regenerated**. `/c/[id]` first-load JS 189,088 → 189,498 B against a 192,512 B ceiling; every budget row PASS. | The residual 0.00495 at 8× is **not the placeholder's**, and it was measured rather than assumed: the report family is not requested by the browser until a briefing renders, so it swaps *after* the document is on screen and re-wraps the report's own `h1` by one line (34 px). That is WO-02's fallback metrics. F1's "not fonts" was right about the 0.07944 and about the page's own faces — `document.fonts.ready` at ~50 ms — but the report face is `unloaded` until t≈837 ms at 8×, which is the frame the `h1` re-wraps on. Also recorded: the first-paint instrument these specs use is a `requestAnimationFrame` loop, and a rAF callback runs after React has flushed the passive effect, so it cannot see a one-frame-late commit at all. The layout-effect half is therefore gated by the CLS entry and by a `flushSync` unit test; the e2e equality that could not fire was deleted rather than shipped green. |
